<template>
  ...
  <div class='relative'>
    ...
    <div class="fresh-loading">
      <lottie :options="defaultOptions" :height="50" :width="70" @animCreated="handleAnimation"/>
    </div>
    <!-- banner -->
    <div class="top-banner" ref = 'banner'>
      <mt-swipe 
      class='SMK banner-box'
      :auto="4000"
    ...
    <div class="fix-recommend" v-if='likeList.length>0'>
      ...
      <div>
        <div
          ...
          <div class="clearfix" ref='tabList' >
            <div 
...
import * as animationData from '../components/loading.json';
...
export default {
  ...
  methods:{
    ...
    cloneTabs () {
      ...
      let topBarHeight = $(this.$refs.topBar).outerHeight()

      let list = this.$refs.tabList
      if(list.length<=0) return ;
      let position = list.map(i=>{
        let top = (i.offsetTop).toFixed(2) - topBarHeight - tab.outerHeight()
        return top || 0
      })
    ...
    changeTab(i){
      let top = this.tabPosition[i]
      this.scrollTo(top).then((res)=>{
        this.tabId = i
      })
    },
    getBefore(){
